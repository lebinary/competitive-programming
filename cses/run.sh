#!/bin/bash
# Compile and run a CSES solution, or build a single-file submission.
#
#   ./cses/run.sh <problem_dir>            compile + run against input.txt
#   ./cses/run.sh <problem_dir> submit     write submit.cpp with cp.h inlined
#
# solution.cpp only contains solve(); everything else lives in cses/cp.h.
# CSES accepts a single file, so `submit` inlines the header for pasting.
#
...
if [ "$MODE" = "submit" ]; then
    ...
    echo "wrote $DIR/submit.cpp - paste this into CSES"
