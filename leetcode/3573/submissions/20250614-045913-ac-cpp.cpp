 ...
 * Difficulty: Medium
 * Status: Accepted
 * Runtime: 232 ms (beats 71.5%)
 * Memory: 176.9 MB (beats 42.6%)
 * Submitted: 2025-06-14 04:59:13 UTC
 * URL: https://leetcode.com/submissions/detail/1663499356/
 */
...
class Solution {
  public:
    long long maximumProfit(vector<int> &prices, int k) {
        ...
        for (int i = 0; i <= n; i++) {
            dp[1][i][k] = LLONG_MIN / 2;
        }
        for (int j = 0; j <= k; j++) {
            dp[2][n][j] = LLONG_MIN / 2;
        }
