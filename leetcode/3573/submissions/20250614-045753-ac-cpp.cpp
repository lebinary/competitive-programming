 ...
 * Difficulty: Medium
 * Status: Accepted
 * Runtime: 221 ms (beats 73.6%)
 * Memory: 177.1 MB (beats 42.6%)
 * Submitted: 2025-06-14 04:57:54 UTC
 * URL: https://leetcode.com/submissions/detail/1663498436/
 */
  ...
  public:
    long long maximumProfit(vector<int> &prices, int k) {
        ...
        for (int i = 0; i <= n; i++) {
            for (int j = 0; j <= k; j++) {
                if (i >= n || j <= 0) {
                    dp[1][i][j] = LLONG_MIN / 2;  // buy position invalid
                    dp[2][i][j] = LLONG_MIN / 2;  // sell position invalid
                }
            }
        }
